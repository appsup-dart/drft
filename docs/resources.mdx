...
Resources are the building blocks of infrastructure in DRFT. This guide explains how to create and use resources.
...
Some resources represent external infrastructure that cannot be created, updated, or deleted through DRFT. These are called **read-only resources** and extend the `ReadOnlyResource` base class.
...
`ReadOnlyResource` is automatically detected by the planner and executor, so no additional configuration is needed.
...
Note: Providers do **not** need to handle `createResource`, `updateResource`, or `deleteResource` for read-only resources, as the planner and executor guarantee these methods will never be called for `ReadOnlyResource` instances.
...
9. **Data Sources**: Extend `ReadOnlyResource` for read-only external resources instead of `Resource`
